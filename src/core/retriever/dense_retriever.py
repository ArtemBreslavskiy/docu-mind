...
class DenseRetriever(BaseRetriever):
    def __init__(self, config: RetrieverConfig, embedder: BaseEmbedder, store: BaseVectorStore):
        self.config = config
    ...
    def retrieve_with_filter(self, query, filter_key=None, filter_value=None, filter_json=None, k=5) \
            -> list[SearchResult]:
        query_emb = self.embedder.embed([query])[0]
        chunk_score_pairs = self.store.search(query_emb, k=k * self.config.filter_oversample_factor)
